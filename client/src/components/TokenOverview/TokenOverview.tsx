...
const TokenOverview = () => {
  ...
  useEffect(() => {
    ...
    const fetchTokenData = async () => {
      try {
        const response = await fetch(`${baseUrl}tokens?symbol=${symbol.toUpperCase()}`);
  ...
  useEffect(() => {
    ...
    const fetchHistoricalData = async () => {
      try {
        const response = await fetch(`https://api.binance.com/api/v3/klines?symbol=${symbol.toUpperCase()}USDT&interval=1s&limit=1000`);
    ...
    <div className="flex flex-col gap-4 text-3xl font-normal px-[15%] py-[2%] h-full">
      <div className="flex flex-col gap-10 bg-lightnav dark:bg-darklabel rounded-md py-[2%] px-[2%]">
        <div className="flex justify-start items-center gap-3">
          ...
          <h1 className="font-semibold">{tokenMetadata?.name} <span className="font-thin text-gunmetal dark:text-stone">{tokenMetadata.symbol}</span></h1>
        </div>
        <div className="flex flex-wrap text-center gap-6 text-black text-lg">
          ...
          <div className="bg-lilac rounded-lg p-4">
            ...
            <p>{tokenData.circulating_supply} <span className="text-darklabel">{tokenMetadata.symbol}</span></p>
          </div>
        <div className="flex flex-col flex-end flex-grow bg-lightlisthov dark:bg-darkbtnhov text-start text-black dark:text-stone rounded-lg p-4">
          ...
          <p>1h: <span className={tokenData.quote.USD.percent_change_1h < 0 ? 'text-red' : 'text-forest'}>{formatPriceChange(tokenData.quote.USD.percent_change_1h)}</span></p>
          <p>24h: <span className={tokenData.quote.USD.percent_change_24h < 0 ? 'text-red' : 'text-forest'}>{formatPriceChange(tokenData.quote.USD.percent_change_24h)}</span></p>
          <p>7d: <span className={tokenData.quote.USD.percent_change_7d < 0 ? 'text-red' : 'text-forest'}>{formatPriceChange(tokenData.quote.USD.percent_change_7d)}</span></p>
          <p>30d: <span className={tokenData.quote.USD.percent_change_30d < 0 ? 'text-red' : 'text-forest'}>{formatPriceChange(tokenData.quote.USD.percent_change_30d)}</span></p>
        </div>
        </div>
